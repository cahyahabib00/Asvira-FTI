...
# ========================================
# ASVIRA FTI - UPLOAD TO VPS SCRIPT
# ========================================
...
read -p "Username VPS (default: root): " VPS_USER
...
if scp deploy-production.sh update-production.sh monitor-production.sh DEPLOYMENT.md "$VPS_USER@$VPS_IP:/root/"; then
...
if [[ $UPLOAD_PROJECT =~ ^[Yy]$ ]]; then
    ...
    if scp asvira-project.tar.gz "$VPS_USER@$VPS_IP:/root/"; then
...
apt install -y nginx mysql-server php8.1 php8.1-fpm php8.1-mysql php8.1-xml php8.1-mbstring php8.1-curl php8.1-zip php8.1-redis composer git redis-server htop iotop nethogs jpegoptim optipng ufw certbot python3-certbot-nginx

# Extract project if uploaded
if [ -f "asvira-project.tar.gz" ]; then
    print_status "Extracting project..."
    tar -xzf asvira-project.tar.gz -C /var/www/
    mv /var/www/Asvira-FTI /var/www/asvira
    rm asvira-project.tar.gz
else
    # Clone from GitHub
    print_status "Cloning from GitHub..."
    git clone https://github.com/cahyahabib00/Asvira-FTI.git /var/www/asvira
fi

# Set permissions
chown -R www-data:www-data /var/www/asvira
chmod -R 755 /var/www/asvira
chmod -R 775 /var/www/asvira/storage
chmod -R 775 /var/www/asvira/bootstrap/cache
...
sed -i "s/APP_URL=http:\/\/localhost/APP_URL=https:\/\/$VPS_DOMAIN/" .env

# Database configuration
sed -i "s/DB_DATABASE=.*/DB_DATABASE=asvira_prod/" .env
sed -i "s/DB_USERNAME=.*/DB_USERNAME=asvira_user/" .env
sed -i "s/DB_PASSWORD=.*/DB_PASSWORD=Asvira2024!@#/" .env
...
# Setup database
mysql -e "CREATE DATABASE IF NOT EXISTS asvira_prod CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
mysql -e "CREATE USER IF NOT EXISTS 'asvira_user'@'localhost' IDENTIFIED BY 'Asvira2024!@#';"
mysql -e "GRANT ALL PRIVILEGES ON asvira_prod.* TO 'asvira_user'@'localhost';"
...
# Configure Nginx
cat > /etc/nginx/sites-available/asvira << 'NGINX_EOF'
server {
    listen 80;
...
# Enable site
ln -sf /etc/nginx/sites-available/asvira /etc/nginx/sites-enabled/
...
if scp remote-setup.sh "$VPS_USER@$VPS_IP:/root/"; then
